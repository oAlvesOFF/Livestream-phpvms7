...
class RouteServiceProvider extends ServiceProvider
{
    ...
    public function map()
    {
        ...
        $this->mapLiveStreamRoutes();
    }

    /**
     * LiveStream module routes — registered here so they always load
     * regardless of whether the module is enabled in the database.
     */
    private function mapLiveStreamRoutes()
    {
        Route::group([
            'middleware' => ['web'],
            ...
            Route::group(['middleware' => ['auth']], function () {
                Route::get('live/profile', 'LiveStreamProfileController@index')
                    ->name('livestream.profile.index');
                Route::post('live/profile', 'LiveStreamProfileController@store')
                    ->name('livestream.profile.store');
            });

            // Public passenger panel
            Route::get('live/passenger/{pirep_id}', function ($pirep_id) {
                // Delegate to module controller if available, otherwise show simple page
                try {
                    $controller = app(\Modules\LiveStream\Http\Controllers\Frontend\PassengerPanelController::class);
                    return $controller->show($pirep_id);
                } catch (\Exception $e) {
                    return response('Passenger panel unavailable.', 503);
                }
            })->name('livestream.passenger.show');

            Route::post('live/passenger/{pirep_id}/interact', function ($pirep_id, \Illuminate\Http\Request $request) {
                try {
                    $controller = app(\Modules\LiveStream\Http\Controllers\Frontend\PassengerPanelController::class);
                    return $controller->interact($request, $pirep_id);
                } catch (\Exception $e) {
            ...
            // OBS Overlay
            Route::get('live/overlay/{pirep_id}', function ($pirep_id) {
                try {
                    $controller = app(\Modules\LiveStream\Http\Controllers\Frontend\OverlayController::class);
                    return $controller->show($pirep_id);
                } catch (\Exception $e) {
                    return response('Overlay unavailable.', 503);
                }
